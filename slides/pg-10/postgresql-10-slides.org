...
sono le principali.
#+end_quote
Il processo principale è denominato *postmaster*; ogni volta che questo riceve una richiesta di connessione si effettua una /fork/ di un processo *postgres* (denominato anche *backend*) delegato a gestire la connessione.
Una connessione può essere *TCP/IP* oppure su *socket locale*.
*** Schema a Processi: IPC
Siccome ogni processo è /fisicamente/ isolato, ma piu' connessioni possono dover condividere i dati, PostgreSQL utilizza un'area *shared memory* ove mantiene i dati. Tale zona di memoria è visibile a tutti i processi *postgres*.

La shared memory viene organizzata in */pagine dati/* che rappresentano la copia in memoria dei dati persistenti su disco. Vi sono una serie di processi di utilità che si occupano di scaricare/caricare i dati dalla /shared memory/ e dal disco.
*** File System
...
*** Numeri di versione (usabili)
La funzione ~version()~ restituisce la stringa identifica del server, ma per essere usata all'interno di script e programmi questa deve essere opportunamente trattata con espressioni regolari. Esistono due variabili particolari, accessibili con ~SHOW~, che consentono di ottenere solo la parta /numerica/ della versione o quella interna:

#+begin_src sql
testdb=> SELECT version();

 PostgreSQL 11beta2 on x86_64-unknown-freebsd11.1,
   compiled by FreeBSD clang version 4.0.0 (tags/RELEASE_400/final 297347)
   (based on LLVM 4.0.0), 64-bit
...
--                                ^^^^
#+end_src
*** Il tempo negli statement
Esiste la funzione ~statement_timestamp()~ che ritorna l'istante di inizio di ogni statement, quindi un valore costante all'interno dello stesso statement.
...
- ~VOLATILE~: /default/, modifica il database e può dare risultati diversi anche se inocata con gli stessi argomenti;
...
- ~IMMUTABLE~: non modifica il database e ritorna lo stesso valore con gli stessi argomenti /sempre/. In genere sono funzioni matematiche.

/Una funzione ~IMMUTABLE~ non *deve* accedere al database nemmeno in lettura!/
*Solo le funzioni ~IMMUTABLE~ possono essere usati per costruire indici!*

Alcuni esempi: ~random()~ è ~VOLATILE~, ~now()~ è ~STABLE~, ~version()~ é ~IMMUTABLE~.
...
*** CTE
Una /Common Table Expression (CTE)/ è una forma speciale di statement SQL (~SELECT~, ~INSERT~, ~UPDATE~, ~DELETE~) che viene /attaccato/ mediante una clausola ~WITH~. Ogni componente della clausola ~WITH~ è detto /statement ausiliario/ mentre lo statement alla quale la clausola ~WITH~ è agganciato viene detto /top level statement/.

Concettualmente è come definire una tabella temporanea che può essere usata in uno statement piu' complesso.
*** CTE: read-only e writable
Una CTE che non modifichi i dati (ossia usi solo ~SELECT~) è detta /read-only/, mentre una che modifica i dati è detta /writable/.
*** CTE: esecuzione degli statement ausiliari
Gli statement ausiliari vengono eseguiti *massimo una volta* per la durata della top level statement.
In particolare:
- se sono read-only vengono eseguiti *solo se esplicitamente referenziati*;
- se sono writable vengono *sempre eseguiti una sola volta, anche se non referenziati*.
*** CTE SELECT
#+begin_src sql
> WITH max_sw AS (
...
Infine la FTS /si basa sul supporto linguistico/, ovvero ci deve essere un meccanismo conosciuto per la lingua specificata di ottenere i lexems dal documento originale.
*** Dizionari
...
** pg_upgrade
*** pg_updgrade (pg_migrator)
E' un tool distribuito con PostgreSQL per l'upgrade in-place di una istanza.
Può funzionare in varie modalità, sostanzialmente effetta un upgrade binario da una versione all'altra.

*Richiede il doppio dello spazio di un cluster* (effettua una copia fisica).

Nella modalità *link* non copia i file ma li collega, quindi non richiede il doppio dello spazio, ma se fallisce distrugge la directory del cluster.
*** Accorgimenti iniziali
1. ~pg_upgrade~ deve essere eseguito dalla versione piu' recente
2. i cluster devono essere spenti
3. la ~PGDATA~ di destinazione deve esistere
4. le librerie linkate dinamicamente (es. PL/Java) devono già essere installate sul nodo di destinazione
5. ~pg_upgrade~ avvierà le due istanze e vi ci si collegarà, quindi è bene configurare una connessione senza passowrd.
*** Esempio di upgrade
#+begin_src shell
% sudo -u postgres
        pg_upgrade -b /opt/pg11b2/bin
                   -B /opt/pg11b3/bin
                   -d /mnt/data1/pgdata
                   -D /mnt/data1/pgdata11b3
                   -v
...
Upgrade Complete
----------------
...
*ATTENZIONE: il ~database~ è quello a cui ci è connessi mediante ~pgbouncer~, non quello effettivo!*
...
*** ~SHUTDOWN~
Interrompe il servizio di ~pgbouncer~.
