...
*Il metodo ~trust~ non richiede autenticazione e va rimosso appena possibile!*
...
- host a cui collegarsi;
- database a cui collegarsi (~*~ per tutti);
- username con cui collegarsi;
- password con cui collegarsi.

Il file non deve essere leggibile da altri utenti (es. permessi ~600~).

#+begin_src sh
% cat ~/.pgpass
127.0.0.1:template1:postgres:mySuperSecretPasswordHere
