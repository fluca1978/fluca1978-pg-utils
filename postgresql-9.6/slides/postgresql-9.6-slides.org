...
** TODO Impatti di Vacuum e autovacuum
*** Tabella di eventi
...
Ad ogni modo non è sempre possibile "anticipare" le necessità di paritioning (e occorre ricordarsi che l'ottimizzazione preventiva è spesso causa di mal di testa!).
...
PostgreSQL supporta alcuni meccanismi che, combinati, possono fornire l'infrastruttura di partitioning:
1. */table inheritance/* che permette di ereditare (nel senso OOP) la struttura da una tabella;
2. */rules/* o */triggers/* che permettono di instradare (routing) le query verso la tabella della partizione giusta;
3. */constraint exclusion/* che permette un uso efficace degli indici nel caso di partizionamento.

*ATTENZIONE: in default PostgreSQL percorre tutta la catena di ereditarietà (instradamento automatico)!*

**** PostgreSQL 10 partitioning                                :B_definition:
In PostgreSQL 10 esiste una apposita sintassi che /racchiude/ tutti i passi in uno unico gestendo in maniera molto piu' semplice il partizionamento.
**** End of block                                           :B_ignoreheading:
:PROPERTIES:
:BEAMER_env: ignoreheading
:END:
...
*** Rules o Triggers?
La logica di partitioning può essere implementata con /triggers/ o /rules/, la scelta in molti casi è indifferente.
...
*** Workflow per il partizionamento
Dipende dal dominio dei dati, dalla loro quantità, ecc., ma in generale:
...
Si supponga di essere nel caso "difficile": dati già presenti.
...
- ~partition~: abilita il meccanismo solo in caso di tabelle con ~INHERITS~.
...
#+end_src
* TODO Configurazione del Server
** Configurazione
*** postgresql.conf
...
E' un meccanismo di join fra tabelle.
...
----------------------------------------------------------------
 Seq Scan on persona  (cost=0.00..244.00 rows=556 width=42)
   Filter: ((nome ~~ 'Nome2%'::text) AND (sesso = 'M'::bpchar))
#+end_src

Il sistema indica che verranno estratte ~556~ tuple, ma come fa a saperlo?
...
Quindi nella query di esempio la selettività è data dalla funzione ~eqsel~.
...
*** Histograms
Nel caso della colonna ~nome~ i valori da usare sono in un /istogramma/ che riporta dei "bucket" di valori ipotizzandoli a distribuzione costante (ossia a pari frequenza):
...
In questo caso ci sono *11 bukect con ~Nome2%~ su un totale di 100 buckets* (si può usare ~array_upper()~ allo scopo), quindi questo significa che la selettività è data da ~11 / 100 = 0.11~.
...
A questo punto il numero delle tuple è dato dal numero delle tuple totali a cui viene applicato l'indice di selettività:
...
*** Il numero dei valori nell'istogramma
Non è un caso che il numero di valori dell'istogramma siano 100: quello è il default per una colonna scalare.
E' possibile aumentare il numero dei valori:

#+begin_src sql
> ALTER TABLE persona ALTER COLUMN nome SET STATISTICS 200;
> EXPLAIN  SELECT * FROM persona 
    WHERE sesso = 'F' AND nome like 'Nome1%';
                           QUERY PLAN                           
----------------------------------------------------------------
 Seq Scan on persona  (cost=0.00..244.00 rows=553 width=42)
   Filter: ((nome ~~ 'Nome1%'::text) AND (sesso = 'F'::bpchar))
#+end_src

In questo caso il numero di bucket si raddoppia (~200~) ma anche il numero di campioni con ~Nome1~, o meglio si hanno ~23~ bucket che forniscono una selettività di ~23/200 = 0.115~ che quindi porta ad un numero di tuple:
  ~10000 * 0.5 * 0.115 = 575~
...
*** Calcolo della selettività
Ci sono 27 valori /MCVs/ minori di 28, per una somma di frequenze complessiva di ~0.5412~, che produce un numero di tuple pari a ~10000 * 0.5412 = 5412~.
...
EXPLAIN SELECT * 
   ...
   AND nome like 'Nome1%';
                                  QUERY PLAN                                   
-------------------------------------------------------------------------------
 Seq Scan on persona  (cost=0.00..269.00 rows=311 width=46)
   Filter: ((eta < 28) AND (nome ~~ 'Nome1%'::text) AND (sesso = 'M'::bpchar))
#+end_Src

Le singole selettività di filtro sono:
- ~0.5412~ per la colonna ~età~;
- ~0.5~ per la colonna ~sesso~;
- ~0.12~ per la colonna ~nome~;

La selettività complessiva è quindi ~0.5412 * 0.5 * 0.12 = 0.032~, che risulta quindi in un numero di tuple in uscita pari a ~10000 * 0.032 = 320~.
...
2. affida alla funzione stabilita dall'operatore (~pg_operator~) il compito di restituire l'indice di selettività:
   - nel caso di /euqlità/ si controllano i /Most Common Values/:
   ...
   - nel caso di non equalità si controlla l'/istogramma/ dei valori, supporti a frequenza costante:
...
> EXPLAIN SELECT * 
   ...
   WHERE eta < 28;
                         QUERY PLAN                          
-------------------------------------------------------------
 Seq Scan on persona  (cost=0.00..219.00 rows=5618 width=46)
   Filter: (eta < 28)
#+end_Src
